...
class PopularFoodPage extends StatefulWidget {
  final int pageId; // Declare the pageId as a final variable

  const PopularFoodPage({Key? key, required this.pageId}) : super(key: key); // Constructor with required pageId
      ...
      body: Column(
        children: [
          ElevatedButton(
            onPressed: () {
              setState(() {
                _showFoodItems = !_showFoodItems;
              });
            },
            child: Text(_showFoodItems ? 'Hide Food Items' : 'Show Food Items'),
          ),
          _showFoodItems
              ? Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return FoodItemCard(index: index);
              },
            ),
          )
              : Center(
            child: BigText(text: 'Click the button to see food items'),
          ),
        ],
      ),
    );
...
class FoodItemCard extends StatelessWidget {
  ...
  const FoodItemCard({Key? key, required this.index}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var foodItem = SlidefoodItems[index];
    ...
    return GestureDetector(
      ...
      child: Card(
        ...
        child: ListTile(
          ...
          title: BigText(text: foodItem['name']!),
          subtitle: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ExpandableText(text: foodItem['description']!),
...
class _PopularFoodDetailsState extends State<PopularFoodDetails> {
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.pageId >= 0 && widget.pageId < SlidefoodItems.length) {
      final foodItem = SlidefoodItems[widget.pageId];

      return Scaffold(
        backgroundColor: Colors.white,
        body: Stack(
          children: [
            _buildBackgroundImage(foodItem['image']!),
            _buildAppBar(context),
            _buildFoodDetailsContainer(foodItem),
          ],
        ),
        bottomNavigationBar: _buildBottomNavigationBar(foodItem),
      );
    } else {
  ...
  Positioned _buildFoodDetailsContainer(Map<String, dynamic> foodItem) {
    return Positioned(
      left: 0,
      right: 0,
      bottom: 0,
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            AppColumnSpacebetween(text: foodItem['name']!),
            SizedBox(height: Dimension.height30),
            const BigText(text: "Introduce"),
            SizedBox(height: Dimension.height20),
            Expanded(
              child: SingleChildScrollView(
                child: ExpandableText(text: foodItem['description']!),
              ),
            ),
          ],
        ),
  ...
  Container _buildBottomNavigationBar(Map<String, dynamic> foodItem) {
    return Container(
  ...
  Container _buildAddToCartButton(Map<String, dynamic> foodItem) {
    return Container(
      padding: EdgeInsets.symmetric(
        vertical: Dimension.height20,
        horizontal: Dimension.width20,
